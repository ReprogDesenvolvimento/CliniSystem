...
def insert_paciente(nome, data_nasc, cpf, telefone, email, endereco):
    ...
    cursor.execute("INSERT INTO pacientes (nome, data_nasc, cpf, telefone, email, endereco) VALUES (?, ?, ?, ?, ?, ?)", (nome, data_formatada, cpf, telefone, email, endereco))
...
def get_paciente_by_id(id):
    ...
    cursor.execute("SELECT * FROM pacientes WHERE id_paciente = ?", (id,))# Consulta o paciente com o CPF fornecido

    return cursor.fetchall()# Retorna todos os dados encontrados para o paciente

# Função para inserir um novo médico no banco de dados
def insert_medico(nome, especialidade, crm, telefone, email):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO medicos (nome, especialidade, crm, telefone, email) VALUES (?, ?, ?, ?, ?)", (nome, especialidade, crm, telefone, email))  # Insere os dados do médico
    conn.commit()
    conn.close()
